...
/**
 * Writearticle command
 * @author Tobias Kahse <tobias.kahse@outlook.com>
 */
public class Writearticle extends Command{

    /**
     * Constructs a Writearticle command using a request and a response object.
     * @param request The request object of the request.
     * @param response The response object of the request.
     */
    public Writearticle (HttpServletRequest request, HttpServletResponse response) {
        super(request, response);        
        this.viewFile = "/articleeditor.jsp";
    ...
    public String execute() {       
        ...
        ScopeHandler.getInstance().store(this.request, "title", "Write Article");
        
        try {
            ModelComponentFactory.createModuleComponent(this.request, this.response, "CreateMainMenu").process();
        } catch (Exception ex) {
            Logger.getLogger(Writearticle.class.getName()).log(Level.SEVERE, null, ex);
        }
